...
@Component({
  ...
  templateUrl: './planogram-upload.component.html',
  styleUrls: ['./planogram-upload.component.scss']
})
export class PlanogramUploadComponent implements OnInit {
  ...
  public onSubmit() {
    if (this.file) {
      const upload = this.uploadService.upload([this.file], `planogram/${this.id}`);
